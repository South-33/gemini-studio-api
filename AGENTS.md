...
- Prompts >= 1500 chars can be converted into text file attachments, but keep ENABLE_PROMPT_FILE_UPLOAD=false by default for search/curation pipelines because Gemini Web skips live [google_search] tool execution when prompts are delivered inside .txt file attachments.
