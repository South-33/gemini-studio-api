...
- Prompts >= 1500 chars (configurable via PROMPT_FILE_UPLOAD_THRESHOLD) are automatically converted into text file attachments in clipboard/uploader to prevent DOM contenteditable freezing, with mandatory wait until card spinner clears & send button is enabled.
