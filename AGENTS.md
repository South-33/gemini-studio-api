...
- On Windows RDP disconnect, use `disconnect_rdp.bat` (console redirect) or Virtual Display Driver (`install_virtual_display.ps1`) to prevent DWM GUI suspension from stalling headful Playwright.
...
- Prompts >= 1500 chars (configurable via PROMPT_FILE_UPLOAD_THRESHOLD) are automatically converted into text file attachments in clipboard/uploader to prevent DOM contenteditable freezing. If the prompt contains 'google' or 'search', 'Use google_search right now.' is automatically typed into the raw chat input box alongside the file attachment to trigger live web search.
